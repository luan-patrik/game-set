...
const FloatingMenuBar = ({ editor }: FloatingMenuBarProps) => {
  return (
    <div className='flex max-h-[14rem] flex-col items-start overflow-auto rounded-md border-2 bg-background p-2'>
      <ButtonMenuBar
        className='flex h-auto items-center justify-start gap-4 rounded-md p-2 text-left'
        onClick={() => editor.chain().focus().setParagraph().run()}
      >
        <CaseSensitive className='sm:h-10 sm:w-10' />
        <div>
          <p>Texto</p>
          <span className='text-ellipsis font-normal'>
            Comece a escrever com texto.
          </span>
        </div>
      </ButtonMenuBar>
      <ButtonMenuBar
        className='flex h-auto w-full items-center justify-start gap-4 rounded-md p-2 text-left'
